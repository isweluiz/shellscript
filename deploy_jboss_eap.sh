...
undeploy() {
   ...
   $JBOSS_HOME/bin/jboss-cli.sh -c --controller=jboss-domain.local <<EOF
...
if [ x$INSTANCE_NAME == "x" ]; then
   ...
   $JBOSS_HOME/bin/jboss-cli.sh -c --controller=jboss-domain.local > /tmp/deploy.$WAR_FILE.info <<EOF
